...
export function generateWorkout(args) {
	...
	let sets = SCHEMES[scheme].ratio
		.reduce((acc, curr, index) => {
			...
			return [...acc, ...[...Array(parseInt(curr)).keys()].map(val => (index === 0 ? 'compound' : 'accessory'))];
		}, [])
		.reduce((acc, curr, index) => {
			const muscleGroupToUse = index < arrOfMuscles.length ? arrOfMuscles[index] : arrOfMuscles[index % arrOfMuscles.length];
			return [
				...acc,
				{
					setType: curr,
	...
	const genWOD = sets.map(({ setType, muscleGroup }) => {
		...
		const filteredObj = Object.keys(data).reduce((acc, curr) => {
			if (includedTracker.includes(curr) || !data[curr].muscles.includes(muscleGroup)) {
		...
		const filteredOppList = Object.keys(setType === 'compound' ? accessoryExercises : compoundExercises).filter(val => !includedTracker.includes(val));

		let randomExercise = filteredDataList[Math.floor(Math.random() * filteredDataList.length)] || filteredOppList[Math.floor(Math.random() * filteredOppList.length)];
